...
class Defects(Tester):
    ...
    def analyze(self):
        ...
        if self.allowable_bad_fraction > 0:
            self.allowable_rejected_pixels = int(
                self.totalpixels * self.allowable_bad_fraction
            )
        else:
            self.allowable_rejected_pixels = -1
        ...
        if self.allowable_rejected_pixels > 0:
    ...
    def analyze_dark_defects(self):
        ...
        self.dark_masked_image = numpy.ma.masked_where(
            self.dark_masked_image < darklimit / binned,
            self.dark_masked_image,
            copy=False,
        )
        rejectedpixels = self.totalpixels - numpy.ma.count(self.dark_masked_image)
        self.dark_rejected_pixels = rejectedpixels
        print(
            f"Actual dark pixels rejected: {self.dark_rejected_pixels}/{self.totalpixels}"
        )
        ...
        if self.use_edge_mask:
            ...
            combined_mask = numpy.bitwise_and(
                self.edge_mask, self.dark_masked_image.mask
            )
        ...
        self.dark_mask = numpy.ma.getmaskarray(self.dark_masked_image).astype("uint8")
    ...
    def analyze_bright_defects(self):
        ...
        # clip to 16-bits
        self.bright_masked_image = numpy.ma.MaskedArray.clip(
            self.bright_masked_image, 0, 65535
        )
        ...
        if self.use_edge_mask:
            ...
            combined_mask = numpy.bitwise_and(
                self.edge_mask, self.bright_masked_image.mask
            )
    ...
    def report(self):
        ...
        if self.allowable_rejected_pixels > 0:
            lines.append(
                f"Allowable total bad pixels: {self.allowable_rejected_pixels}"
            )
            lines.append("")
        ...
        lines.append("")
        if self.allowable_dark_pixels > 0:
