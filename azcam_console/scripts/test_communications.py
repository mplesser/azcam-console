...
def test_communications(cycles: int = 100):
    ...
    for loop in range(cycles):
        azcam.db.server.command("controller.test_datalink 1 111 1 ")
        print("Cycle: %6d/%d" % (loop, cycles - 1), end="\r")
    print()

    print("Timing board TestDataLink:")
    for loop in range(cycles):
        azcam.db.server.command("controller.test_datalink 2 222 1")
        print("Cycle: %6d/%d" % (loop, cycles - 1), end="\r")
    print()
    ...
    try:
        for loop in range(cycles):
            azcam.db.server.command("controller.test_datalink 3 333 1")
            print("Cycle: %6d/%d" % (loop, cycles - 1), end="\r")
        print()
